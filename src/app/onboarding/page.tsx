...
const STEPS = [
  { icon: Sparkles, title: "Willkommen", gradient: "gradient-neon" },
  { icon: User, title: "Persönliches", gradient: "gradient-neon" },
  { icon: UtensilsCrossed, title: "Ernährung", gradient: "gradient-orange" },
  { icon: Dumbbell, title: "Sport", gradient: "gradient-neon" },
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center gap-1.5 mb-8">
          {STEPS.map((_, i) => (
            <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-500 ${i <= step ? "gradient-neon" : "bg-white/10"}`} />
          ))}
        </div>
        ...
        {step === 0 && (
          <div className="text-center animate-slide-up">
            <div className="inline-flex gradient-neon rounded-2xl p-5 glow-neon mb-6">
            ...
            <Button onClick={() => setStep(1)} className="mt-8 gradient-neon text-white border-0 rounded-xl px-8 h-12 text-base font-semibold glow-neon-sm hover:opacity-90 transition-opacity">
          ...
          <div className="animate-slide-up space-y-5">
            ...
            <div className="card-dark rounded-xl p-5 space-y-4">
              <div>
                ...
                <Input value={profile.name} onChange={(e) => up({ name: e.target.value })} placeholder="Dein Name" className="bg-white/[0.03] border-[#1A2332] mt-1" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs text-muted-foreground">Alter</Label>
                  <Input type="number" value={profile.age} onChange={(e) => up({ age: parseInt(e.target.value) || 0 })} className="bg-white/[0.03] border-[#1A2332] mt-1" />
                </div>
                <div>
                  ...
                  <Select value={profile.gender} onValueChange={(v) => up({ gender: v as UserProfile["gender"] })}>
                    <SelectTrigger className="bg-white/[0.03] border-[#1A2332] mt-1"><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="number" value={profile.weight} onChange={(e) => up({ weight: parseFloat(e.target.value) || 0 })} className="bg-white/[0.03] border-[#1A2332] mt-1" />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">Größe (cm)</Label>
                  <Input type="number" value={profile.height} onChange={(e) => up({ height: parseInt(e.target.value) || 0 })} className="bg-white/[0.03] border-[#1A2332] mt-1" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs text-muted-foreground">Aufstehen</Label>
                  <Input type="time" value={profile.wakeUpTime} onChange={(e) => up({ wakeUpTime: e.target.value })} className="bg-white/[0.03] border-[#1A2332] mt-1" />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">Schlafen</Label>
                  <Input type="time" value={profile.sleepTime} onChange={(e) => up({ sleepTime: e.target.value })} className="bg-white/[0.03] border-[#1A2332] mt-1" />
                </div>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {GOALS.map((g) => (
                  <button key={g} onClick={() => toggleGoal(g)} className={`rounded-full px-3.5 py-1.5 text-sm transition-all ${profile.goals.includes(g) ? "gradient-neon text-white glow-neon-sm" : "card-dark text-muted-foreground hover:text-foreground"}`}>
                    {g}
                  </button>
                ))}
              </div>
            ...
            <div className="card-dark rounded-xl p-5 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <Input value={dislikedInput} onChange={(e) => setDislikedInput(e.target.value)} placeholder="z.B. Gemüse, Pilze..." className="bg-white/[0.03] border-[#1A2332]" onKeyDown={(e) => e.key === "Enter" && addDisliked()} />
                  <Button onClick={addDisliked} variant="ghost" size="icon" className="card-dark shrink-0">+</Button>
                </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <Input value={allergyInput} onChange={(e) => setAllergyInput(e.target.value)} placeholder="z.B. Laktose, Nüsse..." className="bg-white/[0.03] border-[#1A2332]" onKeyDown={(e) => e.key === "Enter" && addAllergy()} />
                  <Button onClick={addAllergy} variant="ghost" size="icon" className="card-dark shrink-0">+</Button>
                </div>
                <div className="flex flex-wrap gap-1.5 mt-2">
              ...
              <div>
                ...
                <Textarea value={profile.customNotes} onChange={(e) => up({ customNotes: e.target.value })} placeholder="z.B. Ich mag kein Gemüse, esse gerne deftig..." className="bg-white/[0.03] border-[#1A2332] mt-1" rows={3} />
              </div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="animate-slide-up space-y-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {SPORTS.map((s) => (
                  <button key={s} onClick={() => toggleSport(s)} className={`rounded-full px-3.5 py-1.5 text-sm transition-all ${profile.sportPreferences.preferredTypes.includes(s) ? "gradient-neon text-white glow-neon-sm" : "card-dark text-muted-foreground hover:text-foreground"}`}>
                    {s}
                  </button>
                ))}
              </div>
          ...
          <div className="animate-slide-up space-y-5">
            ...
            <div className="card-dark rounded-xl p-5 space-y-4">
              <div>
                ...
                <Input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="AIza..." className="bg-white/[0.03] border-[#1A2332] mt-1" />
              </div>
        ...
        {step > 0 && (
          <div className="flex gap-3 mt-6">
            <Button variant="ghost" onClick={() => setStep(step - 1)} className="flex-1 card-dark rounded-xl h-11">
              <ArrowLeft className="mr-2 h-4 w-4" /> Zurück
            </Button>
            {step < STEPS.length - 1 ? (
              <Button onClick={() => setStep(step + 1)} className="flex-1 gradient-neon text-white border-0 rounded-xl h-11 font-semibold hover:opacity-90 transition-opacity">
                Weiter <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            ) : (
              <Button onClick={finish} disabled={!profile.name || !apiKey} className="flex-1 gradient-neon text-white border-0 rounded-xl h-11 font-semibold glow-neon-sm hover:opacity-90 transition-opacity disabled:opacity-40">
